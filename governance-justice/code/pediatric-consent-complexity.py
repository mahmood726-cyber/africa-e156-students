#!/usr/bin/env python3
"""
Pediatric Consent Complexity — Africa E156 Student Analysis
Group: governance-justice | Paper #33

Condition: pediatric OR child
Location: Africa

This standalone script demonstrates the statistical methods used in this paper.
Run: python pediatric-consent-complexity.py
"""
...
def fetch_trial_count(condition=None, location=None, other=None):
    ...
    params["query.cond"] = "pediatric OR child"
...
def main():
    print("=" * 60)
    print(f"  {'Pediatric Consent Complexity'}")
    print(f"  Africa E156 | Group: governance-justice")
    print("=" * 60)
    print()
    ...
    or_val, lo, hi = odds_ratio_2x2(africa_count, us_count, europe_count, max(1, us_count - africa_count))
    if or_val: print(f"Odds ratio: {or_val:.3f} [{lo:.3f}, {hi:.3f}]")
